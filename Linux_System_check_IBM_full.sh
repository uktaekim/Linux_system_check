...
################################################################################################
# Name of the script : Linux_System_check_IBM_full.sh
...
# This part prepares system environment to use the script "Linux_system_check_IBM_full.sh"
...
if [ `ls -artl / | grep -w IBM_System_check | wc -l` -eq 1 ] ; then
  ...
  mv ./Linux_System_check_IBM_full.sh /IBM_System_check/Linux_System_check_IBM_full.sh
  chmod 700 /IBM_System_check/Linux_System_check_IBM_full.sh
fi
...
echo "== lsmod CHANGES ==" &>> $tmp_file
lsmod &>> /IBM_System_check/Resource/$CHKDATE_NOW/lsmod
diff /IBM_System_check/Resource/$CHKDATE_BEFORE/lsmod /IBM_System_check/Resource/$CHKDATE_NOW/lsmod &>> $tmp_file
echo -e "\n" &>> $tmp_file
...
echo "== BOOTING TIME ==" &>> $tmp_file
if [ $OSCHK -eq 3 ];
then
  systemd-analyze blame | head -n 7 &>> $tmp_file
else
  echo "This version OS doesn't support getting BOOTING TIME." &>> $tmp_file
fi
echo -e "\n" &>> $tmp_file
...
echo "== Parameters CHANGES ==" &>> $tmp_file
sysctl -a | egrep -v "random|inode-nr|inode-state|dentry-state|fs.file-nr|ns_last_pid|netfilter.nf_conntrack_count|kernel.pty.nr" &>> /IBM_System_check/Resource/$CHKDATE_NOW/sysctl_a
diff /IBM_System_check/Resource/$CHKDATE_BEFORE/sysctl_a /IBM_System_check/Resource/$CHKDATE_NOW/sysctl_a &>> $tmp_file
echo -e "\n" &>> $tmp_file
...
echo "== Directory Size CHANGES ==" &>> $tmp_file
du -sh /opt /home /usr /lib /bin /var /sbin /root /boot /sys /etc &>> /IBM_System_check/Resource/$CHKDATE_NOW/du-sh
diff /IBM_System_check/Resource/$CHKDATE_BEFORE/du-sh /IBM_System_check/Resource/$CHKDATE_NOW/du-sh &>> $tmp_file
echo -e "\n" &>> $tmp_file

echo "== lsblk CHANGES ==" &>> $tmp_file
lsblk | grep &>> /IBM_System_check/Resource/$CHKDATE_NOW/lsblk
diff /IBM_System_check/Resource/$CHKDATE_BEFORE/lsblk /IBM_System_check/Resource/$CHKDATE_NOW/lsblk &>> $tmp_file
echo -e "\n" &>> $tmp_file

echo "== Stripe volume CHANGES ==" &>> $tmp_file
lvs -v --segments &>> /IBM_System_check/Resource/$CHKDATE_NOW/stripe
diff /IBM_System_check/Resource/$CHKDATE_BEFORE/stripe /IBM_System_check/Resource/$CHKDATE_NOW/stripe &>> $tmp_file
echo -e "\n" &>> $tmp_file
...
echo "== DEPENDENCY CHECK ==" &>> $tmp_file
rpm -Va --nofiles &>> $tmp_file
echo -e "\n" &>> $tmp_file

echo "== Package status CHANGES ==" &>> $tmp_file
rpm -Va &>> /IBM_System_check/Resource/$CHKDATE_NOW/rpmva
diff /IBM_System_check/Resource/$CHKDATE_BEFORE/rpmva /IBM_System_check/Resource/$CHKDATE_NOW/rpmva &>> $tmp_file
echo -e "\n" &>> $tmp_file
...
echo "== All daemons CHANGES ==" &>> $tmp_file
if [ $OSCHK -eq 3 ];
then
   systemctl list-unit-files &>> /IBM_System_check/Resource/$CHKDATE_NOW/listunitfiles
   diff /IBM_System_check/Resource/$CHKDATE_BEFORE/listunitfiles /IBM_System_check/Resource/$CHKDATE_NOW/listunitfiles &>> $tmp_file
else
   chkconfig --list &>> /IBM_System_check/Resource/$CHKDATE_NOW/chkconfiglist
   diff /IBM_System_check/Resource/$CHKDATE_BEFORE/chkconfiglist /IBM_System_check/Resource/$CHKDATE_NOW/chkconfiglist &>> $tmp_file
fi
echo -e "\n" &>> $tmp_file

echo "== Loaded daemons CHANGES ==" &>> $tmp_file
if [ $OSCHK -eq 3 ];
then
   systemctl list-units  &>> /IBM_System_check/Resource/$CHKDATE_NOW/listunits
   diff /IBM_System_check/Resource/$CHKDATE_BEFORE/listunits /IBM_System_check/Resource/$CHKDATE_NOW/listunits &>> $tmp_file
else
   echo "This version OS doesn't support this command." &>> $tmp_file
fi
echo -e "\n" &>> $tmp_file
...
sar -u -r -d -n DEV 1 10 | grep Average &>> $tmp_file
